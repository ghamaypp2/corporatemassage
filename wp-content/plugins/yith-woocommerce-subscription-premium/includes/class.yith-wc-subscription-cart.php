			...
			if ( YITH_WC_Subscription()->is_subscription( $product_id ) ) {
				...
				if( is_checkout() && 'yes' == get_option('ywsbs_subscription_total_amount', 'no')){
					...
					if( $max_lenght && $max_lenght > 1 ){
						$price_html .= apply_filters('ywsbs_checkout_subscription_total_amount', '<br><small>'.sprintf( __( '(Subscription total for %s %s: %s)', 'yith-woocommerce-subscription' ), $max_lenght, $price_time, wc_price( $price ), $price_time  ) .'</small>' ) ;
					}
				}
			}
