...
if ( ! class_exists( 'YWSBS_Subscription_Helper' ) ) {
	...
	class YWSBS_Subscription_Helper {
		...
		public function before_data_saving( $post_id, $post ) {
			...
			if ( isset( $_POST['ywsbs_safe_submit_field'] ) && 'schedule_subscription' == $_POST['ywsbs_safe_submit_field'] ) {
				...
				if ( isset( $_REQUEST['ywsbs_price_is_per'] ) && isset( $_REQUEST['ywsbs_price_time_option'] ) &&
					...
					YITH_WC_Activity()->add_activity( $post_id, "changed", $status = "success", $order = 0, sprintf( __( 'Changed recurring period from: %s to %s', 'yith-woocommerce-subscription' ), $recurring_old, $recurring_new ) );
				}
				...
				foreach ( $schedule_fields as $key => $value ) {
					...
					if ( $subscription->$key != $new_value ) {
						...
						YITH_WC_Activity()->add_activity( $post_id, "changed", $status = "success", $order = 0, sprintf( __( 'Changed %s from: %s to %s', 'yith-woocommerce-subscription' ), $value, $old_value_formatted, $new_value_formatted ) );
					}
				}
			}
		...
		public function show_info_subscription() {
			add_meta_box( 'ywsbs-info-subscription', __( 'Subscription Info', 'yith-woocommerce-subscription' ), array(
				$this,
				'show_subscription_info_metabox'
			), YITH_WC_Subscription()->post_name, 'normal', 'high' );
		}
		...
		public function show_action_subscription() {
			add_meta_box( 'ywsbs-action-subscription', __( 'Subscription Action', 'yith-woocommerce-subscription' ), array(
				$this,
				'show_subscription_action_metabox'
			), YITH_WC_Subscription()->post_name, 'side', 'high' );
		}
		...
		public function show_activity_subscription() {
			add_meta_box( 'ywsbs-activity-subscription', __( 'Subscription Activities', 'yith-woocommerce-subscription' ), array(
				$this,
		...
		public function show_product_subscription() {
			add_meta_box( 'ywsbs-product-subscription', __( 'Subscription Product', 'yith-woocommerce-subscription' ), array(
				$this,
		...
		public function show_schedule_subscription( $post_type, $post ) {
			...
			add_meta_box( 'ywsbs-schedule-subscription', __( 'Subscription Schedule', 'yith-woocommerce-subscription' ), array(
				$this,
