	...
	function ywsbs_check_renew_order_before_pay( $order ) {
		...
		if ( $is_valid ) {
			...
			if ( ! $order->has_status( $status ) ) {
				...
				yith_subscription_log( sprintf( __( 'New payment request denied because the order #%d is on %s status', 'yith-woocommerce-subscription' ), $order_id, $order->get_status() ), 'subscription_payment' );
